 ...
 * folgendem Aufbau: KSSSSSSPUU K = Kontoartziffer S = Stammnummer P =
 * Prüfziffer U = Unterkontonummer
 * 
 * Die für die Berechnung relevante 6-stellige Stammnummer (Kundennummer)
 * befindet sich in den Stellen 2 bis 7 der Kontonummer, die Prüfziffer in der
 ...
 * die Prüfziffer. Verbleibt nach der Division durch 11 kein Rest, ist die
 * Prüfziffer 0.
 * 
...
public class Checksum17 extends AbstractChecksumValidator {
	...
	protected int calcChecksum(int[] accountNumber) {
		...
		for (int i = 1; i <= getWeights().length; i++) {
			sum += ChecksumUtils.qs(accountNumber[i] * getWeights()[i - 1]);
		}
